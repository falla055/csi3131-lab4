...
enum PagingAlgorithm { FIFO, LRU, CLOCK, COUNT, WORKING_SET, RANDOM};  // For definining paging algorithm used.
...
class MemManage extends EvSched  // Event Scheduling Simulation program
{
	...
	public MemManage(PagingAlgorithm pgAlg, double t0time, double tftime, Seeds sd)
	{
		...
		kernel = new Kernel(NumFrames, pgAlg, sd.dRand);
	...
	private void endPageFault(Process cProcess)	// finished a page fault operation
	{
	   ...
	   KernelFunctions.pageReplacement(cProcess.vpage,cProcess,kernel, clock);
...
class Kernel
{
	...
	public Kernel(int numFrames, PagingAlgorithm pgAlg, int randSeed)
	{
	   int i;
